...
with sync_playwright() as pw:
    ...
    try:
        ...
        k.wait_for_selector('#modal', state='hidden')
        tab(k, 'journal')
        ...
        s.wait_for_selector('#modal', state='hidden')
        ...
        tab(k, 'pay')
        check('Motivation has no owner controls', k.locator('[data-action=sm65Rules],[data-action=sm65Close]').count() == 0 and 'предварительный расчёт' in k.locator('#pages').inner_text().lower())
        tab(s, 'pay')
        ...
        r.wait_for_selector('#modal', state='hidden')
        k.evaluate('window.EKGrowthOS.sync()')
        ...
        for width in [1366, 1024, 768, 390, 375]:
            ...
            tab(k, 'today')
            if k.evaluate('document.documentElement.scrollWidth>innerWidth+1'):
                print('OVERFLOW', k.evaluate("Array.from(document.querySelectorAll('body *')).filter(e=>e.getBoundingClientRect().right>innerWidth+1&&e.getBoundingClientRect().width>0).slice(0,15).map(e=>({tag:e.tagName,cls:e.className,right:e.getBoundingClientRect().right,width:e.getBoundingClientRect().width}))"), flush=True)
                k.screenshot(path=str(ROOT/'evidence/overflow.png'),full_page=True)
