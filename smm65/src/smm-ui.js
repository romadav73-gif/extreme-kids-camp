...
function sm65Motivation(score) {
  const rows = [['Фиксированная часть', score.base, score.base, 'Ставка ' + sm65Money(score.rules.base) + ' за полный месяц'], ['Ежедневные сторис', score.stories.earned, score.stories.max, `${score.stories.done} из ${score.stories.planned} пунктов приняты`], ['План постов', score.posts.earned, score.posts.max, `${score.posts.done} из ${score.posts.planned} пунктов приняты`], ['Видео и съёмки', score.production.earned, score.production.max, `${score.production.done} из ${score.production.planned} пунктов приняты`]];
...
const sm65RulesFields = ['base', 'storyBonus', 'postBonus', 'productionBonus', 'cap', 'postEvery', 'storyFrames', 'storyPlatforms', 'postPlatforms', 'videoPlatforms', 'videoDays', 'shootDays', 'workDays', 'accounts'];
  ...
  if (form.id === 'sm65RulesForm' || form.id === 'sm65CloseForm') {
    ...
    return sm65Run(async () => {
      if (form.id === 'sm65CloseForm') await sm65Send({ action: 'close', month: sm65Month(), amount: Number(form.elements.amount.value), note: form.elements.note.value, expectedRevision: Number(form.dataset.revision) });
        ...
        const rules = {};
        for (const k of ['base', 'storyBonus', 'postBonus', 'productionBonus', 'cap', 'postEvery', 'storyFrames']) rules[k] = Number(form.elements[k].value);
