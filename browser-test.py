...
with sync_playwright() as p:
 def launch(width=1280):
  ...
  ctx.on('page',lambda page:page.on('pageerror',lambda e:print('PAGE_ERROR',str(e),flush=True)))
  return ctx
 ctx=launch();page=ctx.pages[0];errors=[];page.on('pageerror',lambda e:(errors.append(str(e)),print('PAGE_ERROR',str(e),flush=True)));page.on('response',lambda r: print('API_STATUS',r.url.split('/')[-1],r.status,flush=True) if '/api/club/' in r.url else None);page.goto('http://127.0.0.1:8877/')
