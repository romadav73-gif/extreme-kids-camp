...
FEATURE_BUILD = '2026.09.03-role-cabinets'
...
def choose_source() -> Path:
    for path in SOURCE_CANDIDATES:
        ...
        if "const VERSION='6.1.0'" in text and 'ATTENDANCE_INTELLIGENCE_V1_END' in text and 'PAYROLL_TEAM_READABILITY_V1_END' in text:
            return path
    raise SystemExit('No Growth OS 6.2 attendance source found')
...
def patch_html(html: str, css: str, js: str) -> str:
    ...
    html = html.replace('<title>EXTREME KIDS · Growth OS 6.2 Attendance</title>', '<title>EXTREME KIDS · Growth OS 6.3 Team Cabinets</title>')
    html = html.replace('Growth OS <span>6.2</span>', 'Growth OS <span>6.3</span>')
    html = html.replace('GROWTH OS <span>6.2</span>', 'GROWTH OS <span>6.3</span>')
...
def main() -> None:
    ...
    metadata = {
        ...
        'featureVersion': '6.3-role-cabinets',
        'build': FEATURE_BUILD,
        'sha256': digest,
    ...
    for folder in ('growth-os', 'growth-os-6', 'growth61-live'):
        ...
        (out / 'RELEASE.txt').write_text(
            'EXTREME KIDS Growth OS 6.3 Role Access & Personal Cabinets\n'
            f'Build: {FEATURE_BUILD}\nSHA256: {digest}\n',
            encoding='utf-8',
        )
    (ROOT / 'growth6-source' / 'SHA256.txt').write_text(digest + '  v6.3.html\n', encoding='utf-8')
