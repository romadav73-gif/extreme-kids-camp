...
async function s65Fetch(url,options={}){const res=await fetch(url,{cache:'no-store',credentials:'same-origin',...options,headers:{...(options.body?{'Content-Type':'application/json','X-CSRF-Token':s65Csrf}:{}),...options.headers}});let data;try{data=await res.json()}catch{throw Error('Некорректный ответ сервера. Повторите позже.')}if(!res.ok){if(res.status===401&&s65Actor){clearInterval(s65Polling);state=null;location.replace('/login')}throw Object.assign(Error(data.message||'Ошибка обмена.'),{status:res.status,code:data.error})}return data}
...
sofa65V4SilentTouch=function(){touch('Изменение контроля')};
syncNow=function({quiet=false}={}){
 ...
 s65Promise=(async()=>{
  ...
  try{
   ...
   if(e.status===409){const remote=await s65Fetch('/api/state');const desired=s65Changes(),merged=s65MergeIntent(S65.clean(remote.state),desired);s65Conflict={changes:desired,remote:S65.clean(remote.state),paths:merged.conflicts};setSyncStatus('off','Конфликт: сохраните выбор вручную');s65RenderAfterSync();if(!quiet)s65ShowConflict()}
   ...
   return false;
  }finally{syncing=false;s65Promise=null}
 })();return s65Promise;
};
...
renderCurrentView=function(){if(!state)return;s65OldRender();const head=$('#pages');if(head&&s65Conflict){const bar=document.createElement('div');bar.className='toolbar';bar.innerHTML='<b>Есть несогласованные изменения</b><button class="btn btn-primary" data-action="s65Conflict">Разобрать конфликт</button>';head.prepend(bar)}if(head&&currentRole!=='owner')for(const b of head.querySelectorAll('[data-action="editAdminRules"],[data-action="editAdminRates"],[data-action="addAdminSale"],[data-action="addAdminAdjustment"],[data-action="addAdminShift"]')){if(currentRole!=='manager')b.remove()}};
...
window.EK65SecureStatus={version:'6.5-security.1',get pending(){return !!s65Flight||!!s65Conflict}};
